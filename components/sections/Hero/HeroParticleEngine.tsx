...
export default function HeroParticleEngine(){useLayoutEffect(()=>{
 ...
 Object.assign(heat.style,{position:"fixed",width:"0px",height:"0px",borderRadius:"50%",pointerEvents:"none",zIndex:"1",background:"radial-gradient(circle,rgba(255,118,18,.13) 0%,rgba(255,76,8,.07) 28%,rgba(255,55,0,.025) 52%,transparent 72%)",filter:"blur(3px)",transform:"translate(-50%,-50%)",opacity:"0"});
 document.body.appendChild(heat);
 const embers:Ember[]=[];let stopped=false,spawnTimer=rand(70,150),raf=0;
 const make=(x:number,y:number,angle:number,isRing=false,initial=false)=>{
  ...
  const distance=isRing?rand(48,155):rand(35,135),dx=Math.cos(angle)*distance,dy=Math.sin(angle)*distance+(isRing?rand(8,32):rand(12,48)),driftX=rand(-38,38),driftY=rand(-8,30),duration=initial?rand(16000,25000):rand(13000,21000),alpha=rand(.34,.80),scaleEnd=rand(.32,.56);
 ...
 seedAmbient();for(let i=0;i<78;i++)ringPoint();
 const tick=()=>{if(stopped)return;updateHeat();const activeRing=embers.reduce((n,e)=>n+(e.ring?1:0),0);if(ring&&activeRing<78){const count=activeRing<62?Math.random()<.65?2:3:1;for(let i=0;i<count;i++)ringPoint()}raf=requestAnimationFrame(tick)};
