...
export default function RingParticleEmitter(){
 useEffect(()=>{
  ...
  const embers:Ember[]=[];const maxParticles=420;let last=performance.now(),nextBirth=rand(45,95),raf=0,stopped=false;
  const makeParticle=(kind:Kind,x:number,y:number,angle:number,initial=false,releaseAt=0)=>{
   ...
   embers.push({el,x,y,vx:initial?0:(Math.cos(angle)*outward+Math.cos(angle+Math.PI/2)*tangent)*speed,vy:initial?0:(Math.sin(angle)*outward+Math.sin(angle+Math.PI/2)*tangent)*speed+gravity*.08,life:0,maxLife:kind==="ambient"?rand(4200,7800):rand(3200,6200),size,drift:rand(.55,1.65),phase:rand(0,Math.PI*2),opacity:rand(.28,.78)*depth,kind,exit:Math.random()<.68?"edge":"fade",releaseAt,released:!initial});
  };
  const sr=scene.getBoundingClientRect();
  for(let i=0;i<190;i++)makeParticle("ambient",rand(12,sr.width-12),rand(12,sr.height-12),rand(0,Math.PI*2));
  const rr=ring.getBoundingClientRect(),cx=rr.left-sr.left+rr.width/2,cy=rr.top-sr.top+rr.height/2;
  for(let i=0;i<216;i++){const angle=rand(0,Math.PI*2),radius=Math.min(rr.width,rr.height)*rand(.488,.502);makeParticle("ring",cx+Math.cos(angle)*radius,cy+Math.sin(angle)*radius,angle,true,rand(250,2600));}
  const tick=(now:number)=>{
   if(stopped)return;const dt=Math.min(34,now-last);last=now;nextBirth-=dt;
   if(nextBirth<=0){makeParticleAtRing();nextBirth=rand(45,95);}
   ...
   for(let i=embers.length-1;i>=0;i--){const p=embers[i];
    if(p.kind==="ring"&&!p.released){p.releaseAt-=dt;const pulse=.72+Math.sin(seconds*p.drift+p.phase)*.07;p.el.style.transform=`translate3d(${p.x}px,${p.y}px,0) scale(${.72*pulse})`;p.el.style.opacity=(p.opacity*.42).toFixed(3);if(p.releaseAt>0)continue;p.released=true;p.el.style.opacity="0";const a=Math.atan2(p.y-height/2,p.x-width/2),outward=rand(1.2,4.2),tangent=rand(-5.2,5.2),speed=rand(.34,.78),gravity=rand(1.8,4.2);p.vx=(Math.cos(a)*outward+Math.cos(a+Math.PI/2)*tangent)*speed;p.vy=(Math.sin(a)*outward+Math.sin(a+Math.PI/2)*tangent)*speed+gravity*.08;p.life=0;}
