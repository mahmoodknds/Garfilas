...
// Six sizes. The smallest and largest stay rare; sizes 2-5 form the visible body.
...
export default function RingParticleEmitter(){
 useLayoutEffect(()=>{
  ...
  const embers:Ember[]=[],maxParticles=900;let spawnTimer=rand(70,110),raf=0,last=performance.now(),stopped=false;
  const createAnimation=(el:HTMLSpanElement,kind:Kind,x:number,y:number,angle:number,size:number,depth:number,initial:boolean,releaseHold:number)=>{
   const maxLife=kind==="ambient"?rand(11000,17000):rand(9000,14000),distance=kind==="ambient"?rand(35,130):rand(40,165),outward=kind==="ambient"?rand(.45,.9):rand(.48,.9),drift=rand(6,18),down=rand(4,14),sway=rand(8,24),phase=rand(0,Math.PI*2),speed=rand(.45,.76),dx=Math.cos(angle)*distance*outward,dy=Math.sin(angle)*distance*outward+down,frames=[] as Keyframe[];
   const hold=initial&&kind==="ring"?releaseHold:0,holdRatio=hold/(maxLife/speed+hold),baseAlpha=rand(.68,.94);
   for(let i=0;i<=12;i++){const raw=i/12,t=holdRatio>0&&raw<holdRatio?0:(raw-holdRatio)/Math.max(.0001,1-holdRatio),eased=1-Math.pow(1-t,1.5),sx=Math.sin(phase+t*Math.PI*rand(.8,1.5))*sway*(1-t*.25),sy=Math.cos(phase*.73+t*Math.PI)*drift*(1-t*.2),opacity=raw<holdRatio?(.38+depth*.42):(t<.06?Math.max(baseAlpha,t/.06):t>.84?(1-t)/.16:1),scale=.82+(1-t)*.30;frames.push({transform:`translate3d(${x+dx*eased+sx}px,${y+dy*eased+sy}px,0) scale(${scale})`,opacity:(opacity*depth).toFixed(3),offset:raw});}
   const duration=maxLife/speed+hold;const animation=el.animate(frames,{duration,delay:0,easing:"linear",fill:"both"});
   if(kind==="ambient") animation.currentTime=rand(duration*.18,duration*.72);
  ...
  const seedRing=()=>{const r=ring.getBoundingClientRect(),s=scene.getBoundingClientRect(),a=rand(0,Math.PI*2),rad=Math.min(r.width,r.height)*rand(.488,.502);makeParticle("ring",r.left-s.left+r.width/2+Math.cos(a)*rad,r.top-s.top+r.height/2+Math.sin(a)*rad,a,true,rand(150,1900));};for(let i=648;i--;)seedRing();
  const tick=(now:number)=>{if(stopped)return;const dt=Math.min(34,now-last);last=now;spawnTimer-=dt;if(spawnTimer<=0){const count=Math.random()<.62?2:3;for(let i=0;i<count;i++)seedRing();spawnTimer=rand(70,110)}raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>{stopped=true;cancelAnimationFrame(raf);for(const p of embers)p.animation.cancel();layer.remove()};
 },[]);return null;
}
