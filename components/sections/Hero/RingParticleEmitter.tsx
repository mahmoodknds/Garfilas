...
export default function RingParticleEmitter(){
 useEffect(()=>{
  ...
  const makeParticle=(kind:Kind,x:number,y:number,angle:number,initial=false,releaseAt=0)=>{
   ...
   const size=rand(1.0,3.2),depth=rand(.34,1),outward=rand(1.5,5.8),tangent=rand(-6.5,6.5),speed=rand(.42,1.0),gravity=rand(2.5,6.5);
  ...
  // Ambient field: fill the whole scene immediately so the first frame is never empty.
  for(let i=0;i<130;i++){
   const x=rand(18,sr.width-18),y=rand(18,sr.height-18),angle=rand(0,Math.PI*2);
   makeParticle("ambient",x,y,angle,false);
  }

  // Ring population: dense but calm, with individual release times and the exact same motion model as ambient particles.
  const cx=rr.left-sr.left+rr.width/2,cy=rr.top-sr.top+rr.height/2,radius=Math.min(rr.width,rr.height)*rand(.488,.502);
  for(let i=0;i<216;i++){
   const angle=rand(0,Math.PI*2),x=cx+Math.cos(angle)*radius,y=cy+Math.sin(angle)*radius;
   makeParticle("ring",x,y,angle,true,rand(250,2400));
  }
  ...
  const tick=(now:number)=>{
   ...
   for(let i=embers.length-1;i>=0;i--){const p=embers[i];
    if(p.kind==="ring"&&!p.released){p.releaseAt-=dt;if(p.releaseAt>0){const pulse=.7+Math.sin(seconds*p.drift+p.phase)*.08;p.el.style.transform=`translate3d(${p.x}px,${p.y}px,0) scale(${.72*pulse})`;p.el.style.opacity=(p.opacity*.42).toFixed(3);continue;}p.released=true;p.el.style.opacity="0";const a=Math.atan2(p.y-(rr.top-sr.top+rr.height/2),p.x-(rr.left-sr.left+rr.width/2));const outward=rand(1.5,5.8),tangent=rand(-6.5,6.5),speed=rand(.42,1),gravity=rand(2.5,6.5);p.vx=(Math.cos(a)*outward+Math.cos(a+Math.PI/2)*tangent)*speed;p.vy=(Math.sin(a)*outward+Math.sin(a+Math.PI/2)*tangent)*speed+gravity*.08;p.life=0;}
    ...
    const fadeIn=p.kind==="ambient"?Math.min(1,p.life/180):Math.min(1,p.life/220),fadeOut=p.exit==="fade"?Math.min(1,(1-t)/.16):Math.min(1,(1-t)/.1),pulse=.82+Math.sin(seconds*p.drift*1.9+p.phase)*.18,alpha=p.opacity*fadeIn*fadeOut*pulse,scale=.7+(1-t)*.34;
