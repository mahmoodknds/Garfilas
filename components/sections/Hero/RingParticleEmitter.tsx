...
export default function RingParticleEmitter(){
 useEffect(()=>{
  ...
  // Populate the ring itself, but hold each ember until its own release moment.
  for(let i=0;i<216;i++)spawn(true);
  const tick=(now:number)=>{
   if(stopped)return;const dt=Math.min(34,now-last);last=now;acc+=dt;
   if(acc>=1){acc=0;if(Math.random()<.72)spawn();if(Math.random()<.16)spawn();}
