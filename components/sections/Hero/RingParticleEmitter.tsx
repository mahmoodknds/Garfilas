...
export default function RingParticleEmitter(){
 useEffect(()=>{
  ...
  const embers:Ember[]=[];const maxParticles=720;let spawnTimer=rand(70,120),raf=0,last=performance.now(),stopped=false;
  const createAnimation=(el:HTMLSpanElement,kind:Kind,x:number,y:number,angle:number,size:number,depth:number,initial:boolean,releaseHold:number)=>{
   ...
   for(let i=0;i<=12;i++){
    const raw=i/12,t=holdRatio>0&&raw<holdRatio?0:(raw-holdRatio)/Math.max(.0001,1-holdRatio),eased=1-Math.pow(1-t,1.5);
    const sx=Math.sin(phase+t*Math.PI*rand(.8,1.5))*sway*(1-t*.25),sy=Math.cos(phase*.73+t*Math.PI)*drift*(1-t*.2);
    const opacity=raw<holdRatio?.28+depth*.34:(t<.06?t/.06:t>.84?(1-t)/.16:1),scale=.82+(1-t)*.30;
    frames.push({transform:`translate3d(${x+dx*eased+sx}px,${y+dy*eased+sy}px,0) scale(${scale})`,opacity:(opacity*depth*rand(.72,.98)).toFixed(3),offset:raw});
   }
   const animation=el.animate(frames,{duration:maxLife/speed+hold,delay:initial?0:rand(-3200,-500),easing:"linear",fill:"both"});
   animation.onfinish=()=>{if(stopped)return;el.remove();const i=embers.findIndex(p=>p.el===el);if(i!==-1)embers.splice(i,1);};return {animation,maxLife};
  };
  ...
  // Ambient field is visible immediately, already distributed across different life stages.
  for(let i=190;i--;)makeParticle("ambient",rand(18,sr.width-18),rand(18,sr.height-18),rand(0,Math.PI*2));
  const seedRing=()=>{const r=ring.getBoundingClientRect(),s=scene.getBoundingClientRect(),a=rand(0,Math.PI*2),rad=Math.min(r.width,r.height)*rand(.488,.502);makeParticle("ring",r.left-s.left+r.width/2+Math.cos(a)*rad,r.top-s.top+r.height/2+Math.sin(a)*rad,a,true,rand(350,2600));};
  // Ring starts populated, but every ember has its own quiet hold before separating.
