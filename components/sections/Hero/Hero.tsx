...
const ambientSparks: Spark[] = Array.from({ length: 190 }, (_, i) => ({
  x: 2 + ((i * 47) % 96),
  y: 2 + ((i * 71) % 95),
  size: 0.8 + ((i * 19) % 22) * 0.3,
  delay: -((i * 0.49) % 12),
  duration: 5.5 + ((i * 17) % 14) * 0.52,
  opacity: 0.14 + ((i * 7) % 10) * 0.055,
  driftX: -20 + ((i * 23) % 41),
  driftY: -34 + ((i * 29) % 27),
}));
  ...
  return (
    <section className="hero" aria-labelledby="hero-title">
      <style>{`
        ...
        /* Keep the real foreground completely isolated from the particle system. */
        .hero-shell{position:relative;z-index:10!important}
        .hero-mascot,.hero-mascot-frame,.hero-mascot-frame img{position:relative;z-index:11!important;mix-blend-mode:normal!important;opacity:1!important;visibility:visible!important;filter:none!important}
        .hero-mascot-frame{background:transparent!important;isolation:auto!important}
        .hero-copy,.hero-cta,.hero-scroll-cue{position:relative;z-index:14!important}
        .hero-orbit-one{z-index:12!important}
        .hero-glow-main{z-index:9!important}
        ...
        @media(max-width:699px){.hero-ring-atmosphere{width:min(82vw,25rem);top:46%}.hero-mascot,.hero-mascot-frame,.hero-mascot-frame img{z-index:11!important}.hero-ember{box-shadow:0 0 5px rgba(255,211,127,.9),0 0 13px rgba(255,103,10,.45)}}
      ...
      <div className="hero-ring-atmosphere" aria-hidden="true">
        {ringEmbers.map((e,i)=><i key={`ember-${i}`} className="hero-ember" style={{"--angle":`${e.angle}deg`,"--travel":`${e.travel}px`,"--size":`${e.size}px`,"--delay":`${e.delay}s`,"--duration":`${e.duration}s`,"--opacity":e.opacity,"--drift":`${e.drift}deg`} as React.CSSProperties} />)}
        {hotEmbers.map((e,i)=><i key={`hot-${i}`} className="hero-ember hero-ember-hot" style={{"--angle":`${e.angle}deg`,"--travel":`${e.travel}px`,"--size":`${e.size}px`,"--delay":`${e.delay}s`,"--duration":`${e.duration}s`,"--opacity":e.opacity,"--drift":`${e.drift}deg`} as React.CSSProperties} />)}
      </div>
