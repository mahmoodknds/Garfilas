  ...
  return (
    <div className="hero-logo-lockup" aria-label="Garfilas Italian Lasagna">
      <style>{`
        ...
        .hero-italy-flag{position:relative;width:clamp(5.6rem,20vw,6.9rem);height:clamp(.13rem,.48vw,.19rem);margin:.58rem auto 0;display:flex;overflow:hidden;isolation:isolate;clip-path:polygon(0 50%,7% 0,93% 0,100% 50%,93% 100%,7% 100%);mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.55) 7%,#000 16%,#000 84%,rgba(0,0,0,.55) 93%,transparent 100%);-webkit-mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.55) 7%,#000 16%,#000 84%,rgba(0,0,0,.55) 93%,transparent 100%);filter:drop-shadow(0 0 1px rgba(255,232,170,.45)) drop-shadow(0 0 3px rgba(255,122,0,.3));opacity:.95}
        .hero-italy-flag span{height:100%;flex:1 1 33.333%;display:block}
        .hero-italy-green{background:#008c45}.hero-italy-white{background:#f4efe3}.hero-italy-red{background:#c92535}
        ...
        @media(max-width:699px){.hero-product-lockup{width:min(92vw,23rem);margin-top:-.72rem;gap:.48rem}.hero-product-line{min-width:2.2rem;max-width:5rem}.hero-product-name{font-size:clamp(.82rem,4.25vw,1.16rem);letter-spacing:.13em;text-indent:.13em;transform:scaleX(.9)}.hero-italy-flag{width:clamp(5.1rem,23vw,6.25rem);height:.15rem;margin-top:.5rem}.hero-slogan{margin-top:.72rem;font-size:clamp(.68rem,3vw,.82rem)}}
        @media(min-width:700px){.hero-product-lockup{width:min(68vw,28rem);margin-top:-.82rem;gap:.9rem}.hero-product-line{min-width:5rem;max-width:8rem}.hero-product-name{font-size:1.45rem;letter-spacing:.18em;text-indent:.18em}.hero-italy-flag{width:7.1rem;height:.18rem;margin-top:.58rem}.hero-slogan{margin-top:.9rem;font-size:.92rem}}
      `}</style>
