  ...
  return (
    <span id={id} className="group relative isolate inline-flex h-[2.62rem] w-[min(43vw,13.6rem)] min-w-[9.65rem] items-center justify-center overflow-hidden rounded-full border border-[rgba(255,145,35,.9)] bg-[linear-gradient(180deg,rgba(38,18,8,.74),rgba(7,5,4,.9)_55%,rgba(22,10,4,.82))] px-4 text-center shadow-[0_0_2px_rgba(255,235,201,.95),0_0_8px_rgba(255,133,22,.55),0_0_18px_rgba(255,77,0,.24),0_0_32px_rgba(255,63,0,.10),inset_0_1px_0_rgba(255,231,184,.2),inset_0_0_12px_rgba(255,123,15,.09)] animate-[cta-neon-breathe_2s_ease-in-out_infinite] transition-[transform,box-shadow,border-color] duration-300 ease-out hover:-translate-y-px hover:border-[rgba(255,204,116,.98)] active:scale-[.985]" role="presentation">
      <style>{`
        ...
        .garfilas-cta-sweep{position:absolute;inset:-55% -70%;z-index:2;pointer-events:none;opacity:0;transform:translateX(-62%) rotate(18deg);background:linear-gradient(100deg,transparent 0%,rgba(255,238,193,.05) 36%,rgba(255,250,225,.92) 49%,rgba(255,190,74,.34) 53%,rgba(255,255,245,.72) 57%,transparent 70%);filter:blur(1.6px);mix-blend-mode:screen;animation:garfilas-luxury-sweep 6.2s cubic-bezier(.22,.72,.18,1) infinite!important;}
        .garfilas-cta-sweep::after{content:'';position:absolute;inset:42% 0;filter:blur(4px);background:linear-gradient(90deg,transparent,rgba(255,157,45,.42),transparent);}
        @keyframes garfilas-luxury-sweep{0%,58%{opacity:0;transform:translateX(-62%) rotate(18deg)}64%{opacity:.2}68%{opacity:.9}72%{opacity:.55}78%{opacity:.75}86%{opacity:0;transform:translateX(62%) rotate(18deg)}100%{opacity:0;transform:translateX(62%) rotate(18deg)}}
      `}</style>
