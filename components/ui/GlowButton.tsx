  ...
  return (
    <span id={id} className="group relative isolate inline-flex h-[2.62rem] w-[min(43vw,13.6rem)] min-w-[9.65rem] items-center justify-center overflow-hidden rounded-full border border-[rgba(255,145,35,.9)] bg-[linear-gradient(180deg,rgba(38,18,8,.74),rgba(7,5,4,.9)_55%,rgba(22,10,4,.82))] px-4 text-center shadow-[0_0_2px_rgba(255,235,201,.95),0_0_8px_rgba(255,133,22,.55),0_0_18px_rgba(255,77,0,.24),0_0_32px_rgba(255,63,0,.10),inset_0_1px_0_rgba(255,231,184,.2),inset_0_0_12px_rgba(255,123,15,.09)] animate-[cta-neon-breathe_2s_ease-in-out_infinite] transition-[transform,box-shadow,border-color] duration-300 ease-out hover:-translate-y-px hover:border-[rgba(255,204,116,.98)] active:scale-[.985]" role="presentation">
      <style>{`
        ...
        .garfilas-cta-sweep{position:absolute;inset:-12% -20%;z-index:2;pointer-events:none;opacity:0;transform:translateX(-55%) rotate(18deg);background:linear-gradient(100deg,transparent 0%,rgba(255,240,202,.12) 41%,rgba(255,255,248,1) 49%,rgba(255,205,105,.8) 53%,rgba(255,255,250,1) 57%,transparent 64%);filter:blur(.45px);mix-blend-mode:screen;animation:garfilas-luxury-sweep 3s cubic-bezier(.18,.62,.22,1) infinite!important;}
        .garfilas-cta-sweep::after{content:'';position:absolute;inset:44% 0;background:linear-gradient(90deg,transparent,rgba(255,164,48,.95),transparent);filter:blur(1.8px);}
        @keyframes garfilas-luxury-sweep{0%,48%{opacity:0;transform:translateX(-55%) rotate(18deg)}52%{opacity:.45}58%{opacity:1}68%{opacity:.85}76%{opacity:1}88%{opacity:0;transform:translateX(55%) rotate(18deg)}100%{opacity:0;transform:translateX(55%) rotate(18deg)}}
      `}</style>
